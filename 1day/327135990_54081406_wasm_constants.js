...
let kDeclNoLocals = 0; // Section declaration constants
...
let kResizableMaximumFlag = 1; // Function declaration flags

let kDeclFunctionName = 0x01;
let kDeclFunctionImport = 0x02;
let kDeclFunctionLocals = 0x04;
let kDeclFunctionExport = 0x08; // Local types
...
let kTableZero = 0;
let kMemoryZero = 0; // Useful signatures
...
let kExprF64ReinterpretI64 = 0xbf; // Prefix opcodes
...
function assertWasmThrows(values, code) {
  try {
    ...
    assertTrue(Number.isInteger(e['WasmExceptionTag'])); // TODO(kschimpf): Extract values from the exception.

    let e_values = [];
    assertEquals(values, e_values); // Success.
