...
if (this.Intl) {
  ...
  assertEquals('America/Port_of_Spain', df.resolvedOptions().timeZone); // Zone ids with more than 2 parts are accepted and normalized.

  df = new Intl.DateTimeFormat('en-US', {
    'timeZone': 'America/north_Dakota/new_salem'
  });
  assertEquals('America/North_Dakota/New_Salem', df.resolvedOptions().timeZone); // 3-part zone IDs are accepted and normalized.
  ...
  assertEquals(df1.resolvedOptions().timeZone, df2.resolvedOptions().timeZone); // ICU does not recognize East-Indiana. Add later when it does.
  ...
  assertEquals('America/Port-au-Prince', df.resolvedOptions().timeZone); // Accepts Ho_Chi_Minh and treats it as identical to Saigon
  ...
  assertEquals(df1.resolvedOptions().timeZone, df2.resolvedOptions().timeZone); // Throws for invalid timezone ids.
