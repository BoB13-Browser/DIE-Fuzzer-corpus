...
function run_tests(shard) {
  ...
  function check_arguments_template(expected_name) {
    var lines = [`  assertEquals_(${expected_name}.length, arguments.length);`, `  for (var i = 0; i < ${expected_name}.length; i++) {`, `    assertEquals_(${expected_name}[i], arguments[i]);`, `  }`];
    return lines.join("\n");
  }
  ...
  var f_cfg_sloppy = {
    ...
    source_template: function (cfg) {
      var receiver = cfg.f_receiver != undefined ? cfg.f_receiver : "global";
      var do_checks = [`  assertEquals_(${receiver}, this);`, `  ${!cfg.check_new_target ? "// " : ""}assertEquals_(undefined, new.target);`, check_arguments, `  checkStackTrace_([f, test]);`].join("\n");
      var lines = [`function f(a) {`, `  ${inlinable_comment(cfg.f_inlinable)}`, `  counter++;`, `  var expected_args = [${cfg.f_args}];`, do_checks, deopt_template(cfg.deopt_mode), do_checks, `  return 42;`, `}`];
      return lines.join("\n");
    }
  };
  var f_cfg_strict = {
    func_name: 'f',
    source_template: function (cfg) {
      var receiver = cfg.f_receiver != undefined ? cfg.f_receiver : "undefined";
      var do_checks = [`  assertEquals_(${receiver}, this);`, `  ${!cfg.check_new_target ? "// " : ""}assertEquals_(undefined, new.target);`, check_arguments, `  checkStackTrace_([f, test]);`].join("\n");
      var lines = [`function f(a) {`, `  "use strict";`, `  ${inlinable_comment(cfg.f_inlinable)}`, `  counter++;`, `  var expected_args = [${cfg.f_args}];`, do_checks, deopt_template(cfg.deopt_mode), do_checks, `  return 42;`, `}`];
      return lines.join("\n");
    }
  };
  var f_cfg_possibly_eval = {
    ...
    source_template: function (cfg) {
      var receiver = cfg.f_receiver != undefined ? cfg.f_receiver : "global";
      var do_checks = [`  assertEquals_(${receiver}, this);`, `  ${!cfg.check_new_target ? "// " : ""}assertEquals_(undefined, new.target);`, check_arguments, `  checkStackTrace_([f, test]);`].join("\n");
      var lines = [`function f(a) {`, `  ${inlinable_comment(cfg.f_inlinable)}`, `  counter++;`, `  var expected_args = [${cfg.f_args}];`, do_checks, deopt_template(cfg.deopt_mode), do_checks, `  return 42;`, `}`, `var eval = f;`];
      return lines.join("\n");
    }
  };
  var f_cfg_bound = {
    ...
    source_template: function (cfg) {
      var do_checks = [`  assertEquals_(receiver, this);`, `  ${!cfg.check_new_target ? "// " : ""}assertEquals_(undefined, new.target);`, check_arguments, `  checkStackTrace_([f, test]);`].join("\n");
      var lines = [`function f(a) {`, `  "use strict";`, `  ${inlinable_comment(cfg.f_inlinable)}`, `  counter++;`, `  var expected_args = [${cfg.f_args}];`, do_checks, deopt_template(cfg.deopt_mode), do_checks, `  return 42;`, `}`, `var receiver = {a: 153};`, `var bound = f.bind(receiver);`];
      return lines.join("\n");
    }
  };
  var f_cfg_proxy = {
    ...
    source_template: function (cfg) {
      var receiver = cfg.f_receiver != undefined ? cfg.f_receiver : "global";
      var do_checks = [`  assertEquals_(${receiver}, this);`, `  ${!cfg.check_new_target ? "// " : ""}assertEquals_(undefined, new.target);`, check_arguments, `  checkStackTrace_([f, test]);`].join("\n");
      var lines = [`function f(a) {`, `  ${inlinable_comment(cfg.f_inlinable)}`, `  counter++;`, `  var expected_args = [${cfg.f_args}];`, do_checks, deopt_template(cfg.deopt_mode), do_checks, `  return 42;`, `}`, `var p = new Proxy(f, {});`];
      return lines.join("\n");
    }
  };
  var g_cfg_normal = {
    receiver: undefined,
    source_template: function (cfg) {
      var lines = [`function g(a) {`, `  "use strict";`, `  ${inlinable_comment(cfg.g_inlinable)}`, `  var expected_args = [${cfg.g_args}];`, check_arguments, `  return ${cfg.f_name}(${cfg.f_args});`, `}`];
      return lines.join("\n");
    }
  };
  var g_cfg_reflect_apply = {
    ...
    source_template: function (cfg) {
      var lines = [`function g(a) {`, `  "use strict";`, `  ${inlinable_comment(cfg.g_inlinable)}`, `  var expected_args = [${cfg.g_args}];`, check_arguments, `  return Reflect.apply(${cfg.f_name}, the_receiver, [${cfg.f_args}]);`, `}`];
      return lines.join("\n");
    }
  };
  var g_cfg_function_apply = {
    receiver: "the_receiver",
    source_template: function (cfg) {
      var lines = [`function g(a) {`, `  "use strict";`, `  ${inlinable_comment(cfg.g_inlinable)}`, `  var expected_args = [${cfg.g_args}];`, check_arguments, `  return ${cfg.f_name}.apply(the_receiver, [${cfg.f_args}]);`, `}`];
      return lines.join("\n");
    }
  };
  var g_cfg_function_apply_arguments_object = {
    ...
    source_template: function (cfg) {
      ...
      var lines = [`function g(a) {`, `  "use strict";`, `  ${inlinable_comment(cfg.g_inlinable)}`, `  var expected_args = [${cfg.g_args}];`, check_arguments, `  return ${cfg.f_name}.apply(the_receiver, arguments);`, `}`];
      return lines.join("\n");
    }
  };
  var g_cfg_function_call = {
    ...
    source_template: function (cfg) {
      ...
      f_args += cfg.f_args;
      var lines = [`function g(a) {`, `  "use strict";`, `  ${inlinable_comment(cfg.g_inlinable)}`, `  var expected_args = [${cfg.g_args}];`, check_arguments, `  return ${cfg.f_name}.call(${f_args});`, `}`];
      return lines.join("\n");
    }
  };
  ...
  var g_inlinable_variants = [true, false]; // This is to avoid bailing out because of referencing new.target.

  var check_new_target_variants = [
  /*true,*/
  false];
  var deopt_mode_variants = ["none", "f", "g", "test"];
  var f_variants = [f_cfg_sloppy, f_cfg_strict, f_cfg_bound, f_cfg_proxy];
  var g_variants = [g_cfg_normal, //      g_cfg_reflect_apply,
  g_cfg_function_apply, //      g_cfg_function_apply_arguments_object,
  g_cfg_function_call];
