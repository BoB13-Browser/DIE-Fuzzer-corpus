...
assertEquals(0, eval('42; if (false) 1; else 0;')); // IterationStatement [13.7]
...
assertEquals(1, eval('42; var y = 2; for (let x of ["foo", "bar"]) y--;')); // WithStatement [13.11.7]

assertUndef(eval('42; with ({}) ;')); // ES5: 42

assertEquals(1, eval('42; with ({}) 1;')); // SwitchStatement [13.12.11]
...
assertEquals(2, eval('42; switch (0) { case 0: 1; case 666: 2; break; }')); // TryStatement [13.15.8]
...
assertEquals(0, eval('42; while (1) { try { 0; break } catch(e) {666} finally {666} }'));
assertEquals(0, eval('42; while (1) { try { throw "" } catch(e) {666} finally {0; break} }'));
assertEquals(0, eval('42; while (1) { try { throw "" } catch(e) {0; break} finally {666} }'));
assertEquals(0, eval('42; while (1) { try { 666 } finally {0; break} }'));
assertEquals(0, eval('42; while (1) { try { 666; break } finally {0; break} }'));
assertEquals(0, eval('42; lab: try { 666; break lab } finally {0; break lab}'));
assertEquals(undefined, eval('var b = 1; ' + 'outer: while (1) { while (1) { if (b--) 42; else break outer; }; 666 }')); // The following is not what ES6 says, but see ES bug 4540.
