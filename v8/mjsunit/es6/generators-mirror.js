...
function TestGeneratorMirror(g, status, line, column, receiver) {
  ...
  assertTrue(mirror instanceof debug.GeneratorMirror); // Check the mirror properties.
  ...
  assertEquals(status, mirror.status()); // Note that line numbers are 0-based, not 1-based.
...
iter.next();
TestGeneratorMirror(iter, 'closed'); // Test generator with receiver.
