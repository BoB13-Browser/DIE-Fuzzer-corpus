...
for (var constructor of typedArrayConstructors) {
  assertEquals(1, constructor.from.length); // TypedArray.from only callable on this subclassing %TypedArray%
  ...
  }, TypeError); // Converting from various other types, demonstrating that it can
  ...
  assertArrayLikeEquals(constructor.from([1, 2, 3]), [1, 2, 3], constructor);
  var set = new Set([1, 2, 3]);
  assertArrayLikeEquals(constructor.from(set), [1, 2, 3], constructor);
  ...
  assertArrayLikeEquals(constructor.from(generator()), [4, 5, 6], constructor);
  assertThrows(function () {
  ...
  assertArrayLikeEquals(constructor.from(nullIterator), [], constructor);
  var nonObjIterator = {};
  ...
  assertThrows(function () {
    constructor.from([], null);
  }, TypeError); // Ensure iterator is only accessed once, and only invoked once
