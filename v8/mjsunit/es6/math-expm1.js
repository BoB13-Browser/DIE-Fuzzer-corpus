...
assertEquals(-1, Math.expm1(-Infinity)); // Sanity check:
...
assertEquals(Infinity, Math.expm1(709.8)); // Test largest double value.

assertEquals(Infinity, Math.exp(1.7976931348623157e308)); // Cover various code paths.

assertEquals(-1, Math.expm1(-56 * Math.LN2));
assertEquals(-1, Math.expm1(-50)); // Test most negative double value.

assertEquals(-1, Math.expm1(-1.7976931348623157e308)); // Test argument reduction.
// Cases for 0.5*log(2) < |x| < 1.5*log(2).

assertEquals(Math.E - 1, Math.expm1(1));
assertEquals(1 / Math.E - 1, Math.expm1(-1)); // Cases for 1.5*log(2) < |x|.

assertEquals(6.38905609893065, Math.expm1(2));
assertEquals(-0.8646647167633873, Math.expm1(-2)); // Cases where Math.expm1(x) = x.

assertEquals(0, Math.expm1(0));
assertEquals(Math.pow(2, -55), Math.expm1(Math.pow(2, -55))); // Tests for the case where argument reduction has x in the primary range.
// Test branch for k = 0.

assertEquals(0.18920711500272105, Math.expm1(0.25 * Math.LN2)); // Test branch for k = -1.

assertEquals(-0.5, Math.expm1(-Math.LN2)); // Test branch for k = 1.

assertEquals(1, Math.expm1(Math.LN2)); // Test branch for k <= -2 || k > 56. k = -3.

assertEquals(1.4411518807585582e17, Math.expm1(57 * Math.LN2)); // Test last branch for k < 20, k = 19.

assertEquals(524286.99999999994, Math.expm1(19 * Math.LN2)); // Test the else branch, k = 20.
