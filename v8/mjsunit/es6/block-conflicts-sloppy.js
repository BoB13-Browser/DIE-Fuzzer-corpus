...
function CheckException(e) {
  ...
  assertTrue(string.indexOf("has already been declared") >= 0 || string.indexOf("redeclaration") >= 0);
  return 'Conflict';
}
...
function TestAll(expected, s, opt_e) {
  ...
  assertEquals(expected === 'LocalConflict' ? 'NoConflict' : expected, TestGlobal(s, e), "global:'" + msg + "'");
  assertEquals(expected === 'LocalConflict' ? 'NoConflict' : expected, TestFunction(s, e), "function:'" + msg + "'");
  assertEquals(expected === 'LocalConflict' ? 'Conflict' : expected, TestBlock(s, e), "block:'" + msg + "'");
}
...
var letbinds = ["let x;", "let x = 0;", "let x = undefined;", "let x = function() {};", "let x, y;", "let y, x;", "const x = 0;", "const x = undefined;", "const x = function() {};", "const x = 2, y = 3;", "const y = 4, x = 5;", "class x { }"];

function forCompatible(bind) {
  return !bind.startsWith('class');
}

var varbinds = ["var x;", "var x = 0;", "var x = undefined;", "var x = function() {};", "var x, y;", "var y, x;"];
...
for (var l = 0; l < letbinds.length; ++l) {
  ...
  for (var v = 0; v < varbinds.length; ++v) {
    ...
    TestConflict(varbinds[v] + letbinds[l]); // Different level.

    TestConflict(letbinds[l] + '{' + varbinds[v] + '}');
    TestConflict('{' + varbinds[v] + '}' + letbinds[l]);
    TestNoConflict(varbinds[v] + '{' + letbinds[l] + '}');
    TestNoConflict('{' + letbinds[l] + '}' + varbinds[v]); // For loop.
  ...
  for (var k = 0; k < letbinds.length; ++k) {
    ...
    TestConflict(letbinds[k] + letbinds[l]); // Different level.

    TestNoConflict(letbinds[l] + '{ ' + letbinds[k] + '}');
    TestNoConflict('{' + letbinds[k] + '} ' + letbinds[l]); // For loop.
  ...
  TestConflict(funbind + letbinds[l]); // Different level.

  TestNoConflict(letbinds[l] + '{' + funbind + '}');
  TestNoConflict('{' + funbind + '}' + letbinds[l]);
  TestNoConflict(funbind + '{' + letbinds[l] + '}');
  TestNoConflict('{' + letbinds[l] + '}' + funbind); // For loop.
...
for (var v = 0; v < varbinds.length; ++v) {
  ...
  TestLocalConflict(funbind + varbinds[v]); // Different level.

  TestLocalConflict(funbind + '{' + varbinds[v] + '}');
  TestLocalConflict('{' + varbinds[v] + '}' + funbind);
  TestNoConflict(varbinds[v] + '{' + funbind + '}');
  TestNoConflict('{' + funbind + '}' + varbinds[v]); // For loop.
...
TestNoConflict('try {} catch(x) {' + funbind + '}'); // Test conflicting parameter/function bindings.
