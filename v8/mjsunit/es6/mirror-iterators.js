...
function testIteratorMirror(iter, offset, expected, opt_limit) {
  ...
  assertArrayEquals(expected, preview); // Check that iterator has not changed after taking preview.
...
testIteratorMirror(map.entries(), 2, []); // Test with maximum limit.
...
testIteratorMirror(set.entries(), 5, []); // Test with maximum limit.
