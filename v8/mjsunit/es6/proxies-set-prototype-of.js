...
assertSame(Object.setPrototypeOf(proxy, {
  a: 5
}), proxy);
...
(function testProxyTrapInconsistent() {
  ...
  prototype = [2];
  Reflect.setPrototypeOf(proxy2, prototype);
  assertSame(prototype, Reflect.getPrototypeOf(target)); // Prevent getting the target's prototype used to check the invariant.
  ...
  assertSame(prototype, Reflect.getPrototypeOf(target)); // Changing the prototype of a non-extensible target will trigger the
  ...
  assertEquals([3], Reflect.getPrototypeOf(target)); // Setting the prototype of a non-extensible target is fine if the prototype
  // doesn't change.
