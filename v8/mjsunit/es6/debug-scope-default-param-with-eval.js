...
function listener(event, exec_state, event_data, data) {
  ...
  try {
    ...
    if (break_count++ == 0) {
      // Inside eval.
      assertEquals([debug.ScopeType.Eval, debug.ScopeType.Block, debug.ScopeType.Closure, debug.ScopeType.Script, debug.ScopeType.Global], frame.allScopes().map(s => s.scopeType()));
      exec_state.prepareStep(Debug.StepAction.StepOut);
      block_scope = frame.scope(1);
    } else {
      // Outside of eval.
      assertEquals([debug.ScopeType.Block, debug.ScopeType.Local, debug.ScopeType.Script, debug.ScopeType.Global], frame.allScopes().map(s => s.scopeType()));
