...
}, ReferenceError); // Test that hoisting from blocks does happen in global scope
...
assertEquals(1, globalHoisted()); // Also happens when not previously defined
...
var globalUndefinedHoistedDescriptor = Object.getOwnPropertyDescriptor(this, "globalUndefinedHoisted");
...
assertEquals(1, globalUndefinedHoistedDescriptor.value()); // When a function property is hoisted, it should be
// made enumerable.
// BUG(v8:4451)
...
var globalNonEnumerableDescriptor = Object.getOwnPropertyDescriptor(this, "globalNonEnumerable"); // BUG(v8:4451): Should be made non-configurable

assertTrue(globalNonEnumerableDescriptor.configurable);
assertTrue(globalNonEnumerableDescriptor.writable); // BUG(v8:4451): Should be made enumerable

assertFalse(globalNonEnumerableDescriptor.enumerable);
assertEquals(1, globalNonEnumerableDescriptor.value()); // When a function property is hoisted, it should be overwritten and
...
var globalNonWritableDescriptor = Object.getOwnPropertyDescriptor(this, "globalNonWritable"); // BUG(v8:4451): Should be made non-configurable

assertTrue(globalNonWritableDescriptor.configurable); // BUG(v8:4451): Should be made writable

assertFalse(globalNonWritableDescriptor.writable);
assertFalse(globalNonEnumerableDescriptor.enumerable); // BUG(v8:4451): Should be overwritten

assertEquals(false, globalNonWritableDescriptor.value); // Test that hoisting from blocks does happen in an eval
...
let dontHoistEval; // BUG(v8:) This shouldn't hoist and shouldn't throw
...
assertTrue(throws); // When the global object is frozen, silently don't hoist
...
assertThrows(() => hoistWhenFrozen, ReferenceError); // Should be assertFalse BUG(v8:4452)
