...
assertFalse("nonpresent" in proxy); // Test interesting algorithm steps:
...
assertFalse("in_your_dreams" in proxy); // Step 8: Result is converted to boolean.
...
result = "string";
assertTrue("foo" in proxy); // Step 9b i. Trap result must confirm presence of non-configurable properties
// of the target.
...
result = false;
assertThrows("'nonconf' in proxy", TypeError); // Step 9b iii. Trap result must confirm presence of all own properties of
...
assertFalse("in_your_dreams" in proxy); // Regression test for crbug.com/570120 (stray JSObject::cast).
