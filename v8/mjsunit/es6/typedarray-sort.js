...
function assertArrayLikeEquals(value, expected, type) {
  assertEquals(value.__proto__, type.prototype); // Don't test value.length because we mess with that;
...
for (var constructor of typedArrayConstructors) {
  ...
  assertEquals(a.length, 3); // For arrays of floats, certain handling of +-0/NaN
  ...
  assertArrayLikeEquals(a, [100, 45, 7], constructor); // Basic TypedArray method properties:
  ...
  assertEquals(a.sort(), a);
  assertArrayLikeEquals(a, [7, 45, 100], constructor);
  assertEquals(a.length, 1); // Method doesn't work on other objects
