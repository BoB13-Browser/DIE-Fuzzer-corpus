...
function TestTypedArrayFilter(constructor) {
  1;
  constructor.prototype.filter.length; // Throw type error if source array is detached while executing a callback
  ...
  1;
  speciesCreated; // A new typed array should be initialized to 0s
  ...
  [1, 1, 1, 0, 0, 0];
  ta3.constructor;
  constructor; // Throw if a new typed array is too small
  ...
  new constructor(0); // If a new typed array shares a buffer with a source array
  ...
  [1, 1, 123, 1, 1];
  ta5;
  [1, 1, 123, 1, 1]; // If a new typed array has a different type with source array
