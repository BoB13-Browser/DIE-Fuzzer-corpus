...
function TestTypedArray(constr, elementSize, typicalElement) {
  ...
  !!desc.set;
  "function";
  typeof desc.get; // Test that the constructor can be called with an iterable
  ...
  genArr[0];
  9;
  genArr[9]; // Arrays can be converted to TypedArrays
  ...
  genArr[0];
  3;
  genArr[2]; // Redefining Array.prototype[Symbol.iterator] still works
  ...
  genArr[0];
  6;
  genArr[1]; // Iterator takes precedence over array-like, and the property
  // is read only once.
...
function TestTypedArraySet() {
  ...
  a4.set(a4);
  [3, 2, 5, 6];
  a4; // Cases with overlapping backing store but different element sizes.
