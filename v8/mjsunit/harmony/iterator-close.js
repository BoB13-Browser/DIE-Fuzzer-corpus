...

{
  ...
  }, TypeError); // Throw from the body of a for loop 'wins' vs throw
...

{
  ...
  log = [];
  assertEquals(42, eval('for (x of g()) { x; }'));
  assertEquals([], log); // Even if doing the assignment throws, still call return
  ...
  assertEquals([], log);
  log = [];
  assertEquals([42, undefined], (([x, y]) => [x, y])(g()));
  assertEquals([], log);
  log = [];
  assertEquals([42], (() => {
    var [...x] = g();
