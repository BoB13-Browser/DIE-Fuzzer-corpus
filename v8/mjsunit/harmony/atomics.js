...
(function TestBadArray() {
  ...
  var u8ca = new Uint8ClampedArray(sab); // Atomic ops required integer shared typed arrays

  var badArrayTypes = [undefined, 1, 'hi', 3.4, ab, u32a, sab, sf32a, sf64a, u8ca];
  badArrayTypes.forEach(function (o) {
    assertThrows(function () {
      Atomics.compareExchange(o, 0, 0, 0);
    }, TypeError);
    assertThrows(function () {
      Atomics.load(o, 0);
    }, TypeError);
...
(function TestBadIndex() {
  ...
  var si32a2 = new Int32Array(sab, 4); // Non-integer indexes should throw RangeError.

  var nonInteger = [1.4, '1.4', NaN, -Infinity, Infinity, undefined, 'hi', {}];
  nonInteger.forEach(function (i) {
    assertThrows(function () {
      Atomics.compareExchange(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.load(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.store(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.add(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.sub(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.and(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.or(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.xor(si32a, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.exchange(si32a, i, 0);
    }, RangeError);
  }, RangeError); // Out-of-bounds indexes should throw RangeError.

  [-1, 2, 100].forEach(function (i) {
    assertThrows(function () {
      Atomics.compareExchange(si32a, i, 0, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.load(si32a, i);
    }, RangeError);
    assertThrows(function () {
      Atomics.store(si32a, i, 0);
    }, RangeError);
  ...
  }, RangeError); // Out-of-bounds indexes for array with offset should throw RangeError.

  [-1, 1, 100].forEach(function (i) {
    assertThrows(function () {
      Atomics.compareExchange(si32a2, i, 0, 0);
    });
    assertThrows(function () {
      Atomics.load(si32a2, i);
    }, RangeError);
    assertThrows(function () {
      Atomics.store(si32a2, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.add(si32a2, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.sub(si32a2, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.and(si32a2, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.or(si32a2, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.xor(si32a2, i, 0);
    }, RangeError);
    assertThrows(function () {
      Atomics.exchange(si32a2, i, 0);
    }, RangeError);
  }); // Monkey-patch length and make sure these functions still throw.

  Object.defineProperty(si32a, 'length', {
    get: function () {
      return 1000;
    }
  });
...
(function TestGoodIndex() {
  ...
  var testOp = function (op, ia, index, expectedIndex, name) {
    for (var i = 0; i < ia.length; ++i) ia[i] = i * 2;
  ...
  IntegerTypedArrayConstructors.forEach(function (t) {
    ...
    [sta, sta2].forEach(function (array) {
      ...
      for (var i = 0; i < array.length; ++i) {
        ...
        assertEquals(50, array[i], name); // array[i] == 50, CAS will not store
...
(function TestIsLockFree() {
  ...
  assertEquals(true, Atomics.isLockFree(4)); // Sizes that aren't equal to a typedarray BYTES_PER_ELEMENT always return
  // false.
...
(function TestToNumber() {
  IntegerTypedArrayConstructors.forEach(function (t) {
    ...
    [false, true, undefined, valueOf, toString].forEach(function (v) {
      var name = Object.prototype.toString.call(sta) + ' - ' + v; // CompareExchange

      sta[0] = 50;
      assertEquals(50, Atomics.compareExchange(sta, 0, v, v), name); // Store

      assertEquals(v | 0, Atomics.store(sta, 0, v), name);
      assertEquals(v | 0, sta[0], name); // Add

      sta[0] = 120;
      assertEquals(120, Atomics.add(sta, 0, v), name);
      assertEquals(120 + (v | 0), sta[0], name); // Sub

      sta[0] = 70;
      assertEquals(70, Atomics.sub(sta, 0, v), name);
      assertEquals(70 - (v | 0), sta[0]); // And

      sta[0] = 0x20;
      assertEquals(0x20, Atomics.and(sta, 0, v), name);
      assertEquals(0x20 & (v | 0), sta[0]); // Or

      sta[0] = 0x3d;
      assertEquals(0x3d, Atomics.or(sta, 0, v), name);
      assertEquals(0x3d | (v | 0), sta[0]); // Xor

      sta[0] = 0x25;
      assertEquals(0x25, Atomics.xor(sta, 0, v), name);
      assertEquals(0x25 ^ (v | 0), sta[0]); // Exchange
...
(function TestWrapping() {
  IntegerTypedArrayConstructors.forEach(function (t) {
    ...
    for (offset = -range; offset <= range; offset += range) {
      ...
      newValWrapped = t.toRange(newVal);
      assertEquals(val, Atomics.compareExchange(sta, 0, val, newVal), name);
      assertEquals(newValWrapped, sta[0], name); // Store
      ...
      valWrapped = t.toRange(val);
      assertEquals(val, Atomics.store(sta, 0, val), name);
      assertEquals(valWrapped, sta[0], name); // Add
      ...
      valWrapped = t.toRange(val + operand);
      assertEquals(val, Atomics.add(sta, 0, operand), name);
      assertEquals(valWrapped, sta[0], name); // Sub
      ...
      valWrapped = t.toRange(val - operand);
      assertEquals(val, Atomics.sub(sta, 0, operand), name);
      assertEquals(valWrapped, sta[0], name); // There's no way to wrap results with logical operators, just test that
      ...
      assertEquals(valWrapped, sta[0], name); // Or

      sta[0] = val = 0x12;
      operand = 0x22 + offset;
      valWrapped = t.toRange(val | operand);
      assertEquals(val, Atomics.or(sta, 0, operand), name);
      assertEquals(valWrapped, sta[0], name); // Xor
