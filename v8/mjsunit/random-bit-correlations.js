  ...
  for (var predictor_bit = -2; predictor_bit < 32; predictor_bit++) {
    ...
    for (var random_bit = 0; random_bit < 32; random_bit++) {
      for (var ago = 0; ago < kHistory; ago++) {
        ...
        if (ago == 0 && predictor_bit == random_bit) continue; // Enter the new random value into the history
        ...
        assertTrue(chi_squared <= 24); // If the predictor bit is a fixed 0 or 1 then it makes no sense to
