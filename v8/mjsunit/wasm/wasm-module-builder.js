...
class Binary extends Array {
  ...
  emit_section(section_code, content_generator) {
    // Emit section name.
    this.emit_string(section_names[section_code]); // Emit the section to a temporary buffer: its full length isn't know yet.

    let section = new Binary();
    content_generator(section); // Emit section length.

    this.emit_varint(section.length); // Copy the temporary buffer.
    ...
    if (wasm.functions.length > 0) {
      ...
      binary.emit_section(kDeclFunctions, section => {
        ...
        for (let func of wasm.functions) {
          has_names = has_names || func.name != undefined && func.name.length > 0;
