...
function assertWasm(expected, func, ffi) {
  ...
  assertEquals(expected, Wasm.instantiateModuleFromAsm(func.toString(), ffi).caller());
}
...
function TestInt32HeapAccessExternal() {
  ...
  var module = Wasm.instantiateModuleFromAsm(TestInt32HeapAccess.toString(), null, memory);
...
function TestHeapAccessIntTypes() {
  var types = [[Int8Array, 'Int8Array', '>> 0'], [Uint8Array, 'Uint8Array', '>> 0'], [Int16Array, 'Int16Array', '>> 1'], [Uint16Array, 'Uint16Array', '>> 1'], [Int32Array, 'Int32Array', '>> 2'], [Uint32Array, 'Uint32Array', '>> 2']];
...
assertEquals(1, Wasm.instantiateModuleFromAsm(TestFloatHeapAccess.toString()).caller());

function TestFloatHeapAccessExternal() {
  var memory = new ArrayBuffer(1024);
  var memory_float64 = new Float64Array(memory);
  var module = Wasm.instantiateModuleFromAsm(TestFloatHeapAccess.toString(), null, memory);
...
(function () {
  ...
  var module = Wasm.instantiateModuleFromAsm(TestInitFunctionWithNoGlobals.toString());
...
(function () {
  ...
  var module = Wasm.instantiateModuleFromAsm(TestExportNameDifferentFromFunctionName.toString());
...
function TestForeignFunctions() {
  ...
  var module = Wasm.instantiateModuleFromAsm(AsmModule.toString(), foreign, null);
...
function TestForeignVariables() {
  ...
  function TestCase(env, i1, f1, i2, f2) {
    ...
    var module = Wasm.instantiateModuleFromAsm(AsmModule.toString(), env);
  ...
  TestCase({
    ...
    bar: 234.5,
    baz: 345.7
  }, 123, 234.5, 345, 345.7); // Check partial operation.

  TestCase({
    baz: 345.7
  }, 0, NaN, 345, 345.7); // Check that undefined values are converted to proper defaults.

  TestCase({
    qux: 999
  }, 0, NaN, 0, NaN); // Check that an undefined ffi is ok.

  TestCase(undefined, 0, NaN, 0, NaN); // Check that true values are converted properly.

  TestCase({
    foo: true,
    bar: true,
    baz: true
  }, 1, 1.0, 1, 1.0); // Check that false values are converted properly.

  TestCase({
    foo: false,
    bar: false,
    baz: false
  }, 0, 0, 0, 0); // Check that null values are converted properly.

  TestCase({
    foo: null,
    bar: null,
    baz: null
  }, 0, 0, 0, 0); // Check that string values are converted properly.
  ...
  TestCase({
    ...
    bar: '234',
    baz: '456.1'
  }, 255, 234, 456, 456.1, 456); // Check that Date values are converted properly.

  TestCase({
    foo: new Date(123),
    bar: new Date(456),
    baz: new Date(789)
  }, 123, 456, 789, 789); // Check that list values are converted properly.

  TestCase({
    foo: [],
    bar: [],
    baz: []
  }, 0, 0, 0, 0); // Check that object values are converted properly.

  TestCase({
    foo: {},
    bar: {},
    baz: {}
  }, 0, NaN, 0, NaN); // Check that getter object values are converted properly.
  ...
  TestCase({
    ...
    bar: o.foo,
    baz: o.foo
  }, 123, 123.4, 123, 123.4); // Check that getter object values are converted properly.
  ...
  TestCase(o, 0, NaN, 123, 123.4); // Check that objects with valueOf are converted properly.
  ...
  TestCase({
    ...
    bar: o,
    baz: o
  }, 99, 99, 99, 99); // Check that function values are converted properly.

  TestCase({
    foo: TestCase,
    bar: TestCase,
    qux: TestCase
  }, 0, NaN, 0, NaN); // Check that a missing ffi object is safe.
...
(function () {
  ...
  var m = Wasm.instantiateModuleFromAsm(TestByteHeapAccessCompat.toString());
  m.store(0, 20);
  m.store(4, 21);
  m.store(8, 22);
  m.storeb(20, 123);
