...
function instantiate(sig, body) {
  ...
  var func = builder.addFunction("", sig).addBody(body);
...
assertVerifies(kSig_i, [kExprI8Const, 0]); // Arguments aren't allow to start functions.
...
(function testInvalidIndex() {
  ...
  var builder = new WasmModuleBuilder();
  var func = builder.addFunction("", kSig_v_v).addBody([kExprNop]);
...
(function testRun() {
  ...
  var func = builder.addFunction("", kSig_v_v).addBody([kExprI8Const, 0, kExprI8Const, 77, kExprI32StoreMem, 0, 0]);
  builder.addStart(func.index);
...
(function testStartFFI() {
  ...
  var func = builder.addFunction("", sig_index).addBody([kExprCallImport, kArity0, 0]);
  builder.addStart(func.index);
