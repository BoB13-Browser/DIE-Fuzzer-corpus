...
(function CompileFunctionsTest() {
  ...
  for (i = 0; i < 1000; i++) {
    builder.addFunction("sub" + i, kSig_i_i).addBody([// --
    kExprGetLocal, 0, // --
    kExprI32Const, i % 61, // --
    kExprI32Sub]) // --
    .exportFunc();
  ...
  assertModule(module, kPageSize); // Check the properties of the functions.
...
(function CallFunctionsTest() {
  var builder = new WasmModuleBuilder();
  var f = [];
  f[0] = builder.addFunction("add0", kSig_i_ii).addBody([kExprGetLocal, 0, // --
  kExprGetLocal, 1, // --
  kExprI32Add]).exportFunc();
  builder.addMemory(1, 1, true);

  for (i = 1; i < 256; i++) {
    f[i] = builder.addFunction("add" + i, kSig_i_ii).addBody([// --
    kExprGetLocal, 0, // --
    kExprGetLocal, 1, // --
