...
function testCallImport(func, check) {
  ...
  builder.addFunction("main", sig_index).addBody([kExprGetLocal, 0, // --
  kExprGetLocal, 1, // --
  kExprCallImport, 2, 0]) // --
  .exportAs("main");
...
function check_FOREIGN_SUB(r, a, b) {
  ...
  assertTrue(was_called); //    assertEquals(global, params[0]);  // sloppy mode
...
function testCallBinopVoid(type, func, check) {
  ...
  builder.addFunction("main", makeSig_r_xx(kAstI32, type)).addBody([kExprGetLocal, 0, // --
  kExprGetLocal, 1, // --
  kExprCallImport, 2, 0, // --
  kExprI8Const, 99]).exportFunc("main");
...
testCallBinopVoid(kAstI32); // TODO testCallBinopVoid(kAstI64);
...
function testCallPrint() {
  ...
  builder.addFunction("main", makeSig_r_x(kAstF64, kAstF64)).addBody([kExprI8Const, 97, // --
...
function testCallImport2(foo, bar, expected) {
  ...
  builder.addFunction("main", kSig_i).addBody([kExprCallImport, kArity0, 0, // --
