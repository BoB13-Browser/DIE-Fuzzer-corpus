...
var last_func_index = builder.addFunction("exec_unreachable", kSig_v_v).addBody([kExprUnreachable]);
var illegal_func_name = [0xff];
var func_names = ["☠", illegal_func_name, "some math: (½)² = ¼", ""];
var expected_names = ["exec_unreachable", "☠", null, "some math: (½)² = ¼", "", "main"];

for (var func_name of func_names) {
  last_func_index = builder.addFunction(func_name, kSig_v_v).addBody([kExprCallFunction, kArity0, last_func_index]).index;
}

builder.addFunction("main", kSig_v_v).addBody([kExprCallFunction, kArity0, last_func_index]).exportFunc();
...
(function testFunctionNamesAsString() {
  ...
  try {
    ...
    for (var i = 0; i < names.length; ++i) {
      ...
      assertTrue(line.startsWith(expected_start), "should start with '" + expected_start + "': '" + line + "'");
    }
  }
