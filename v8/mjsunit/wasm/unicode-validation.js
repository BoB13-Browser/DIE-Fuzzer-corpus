...
function isValidUtf8(arr) {
  ...
  try {
    ...
    for (var i = 0; i < s.length; ++i) if ((s.charCodeAt(i) & 0xfffe) == 0xfffe) return false;
...
function checkImportsAndExports(imported_module_name, imported_function_name, internal_function_name, exported_function_name, shouldThrow) {
  var builder = new WasmModuleBuilder();
  builder.addImportWithModule(imported_module_name, imported_function_name, kSig_v_v);
  builder.addFunction(internal_function_name, kSig_v_v).addBody([kExprCallImport, kArity0, 0]).exportAs(exported_function_name); // sanity check: does javascript agree with out shouldThrow annotation?

  assertEquals(shouldThrow, !isValidUtf8(imported_module_name) || !isValidUtf8(imported_function_name) || !isValidUtf8(exported_function_name), "JavaScript does not agree with our shouldThrow expectation");
  ...
  assertEquals(shouldThrow, hasThrown, "Should throw validation error on invalid names");
}
